...
def tabu_search(problem, steps=100, delta=1, initial=None, tabuSize=5):
    ...
    for step in range(steps):
        ...
        for candidate in neighborHood:
            if candidate not in tabuList and problem.compareEvaluations(problem.objective(bestCandidate), problem.objective(candidate)) > 0:
                bestCandidate = candidate
        if problem.compareEvaluations(problem.objective(best), problem.objective(bestCandidate)) > 0:
            best = bestCandidate
        tabuList.append(bestCandidate)
